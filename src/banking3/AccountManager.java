...
public class AccountManager implements ICustomDefine {
    ...
    public void makeAccount() {
        ...
        System.out.print("선택:");
        int type = parseIntLine(); // 문자 입력시 재입력 유도
    ...
    public void depositMoney() {
        ...
        int amount;
        try {
            amount = parseIntLine(); // 문자 입력시 재입력 유도
        } catch (Exception e) {
            // theoretically parseIntLine이 내부에서 처리하지만, 명시적 try~catch 예시
            System.out.println("숫자만 입력할 수 있습니다.");
            System.out.println();
            return;
        }

        // 조건문(if)에 의한 예외처리: 음수 금지
        if (amount < 0) {
        ...
        accounts[idx].deposit(amount); // 타입별 이자 규칙 적용(입금시에만)
        System.out.println("입금이 완료되었습니다.");
        System.out.println();
    }

    /* ===== 출금: 1000원 단위 + 음수 금지 + 잔고부족 시 YES/NO ===== */
    public void withdrawMoney() {
        ...
        // 잔고보다 많은 금액 요청 시 YES/NO 분기
        int balance = accounts[idx].getBalance();
        if (amount > balance) {
            System.out.println("잔고가 부족합니다. 금액전체를 출금할까요?");
            System.out.print("YES/NO: ");
            if (readYesNo()) {
                // 전체 출금(요구사항 특례: 전체출금은 1000원 단위 체크 없이 진행)
                accounts[idx].withdraw(balance);
    ...
    /** 숫자만 허용. 문자 입력 시 재입력 유도 (try~catch 사용) */
