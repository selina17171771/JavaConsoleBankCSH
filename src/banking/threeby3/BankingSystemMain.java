    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                ...
                if (choice < MAKE || choice > GAME) {    // 개발자 정의 예외(지정 정수 이외)
                    throw new MenuSelectException("메뉴는 1~8 사이의 값만 가능합니다.");
                }
                ...
                switch (choice) {
                    ...
                    case SAVEOPT -> {
                    	manager.showSaveOptionMenu();
                    	int sub = Integer.parseInt(sc.nextLine().trim());
                    	if (sub == 1) manager.startAutoSave();
                    	else if (sub == 2) manager.stopAutoSave();
                    	else System.out.println("올바른 값을 입력하세요. \n");
                    }
                    case GAME -> puzzle3x3.start(sc);
                    case EXIT    -> {
